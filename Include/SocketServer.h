...
// 		do 
// 		{
// 			nRecvNum = recv(s,(char *)szTemp, 8192,0);
// 			nTotalRecv += nRecvNum;
// 			
// 			tLastAction = GetExactTime();
...
// 		} while (true);
// 		TraceMsgA("%s Recv %d bytes.\n", __FUNCTION__, nTotalRecv);
...
// 	}
	...
	int Recv()
	{
		...
		int nRecvNum = recv(s, (char *)szTemp, 1024 * 8, 0);
		tLastAction = GetExactTime();
		if (nRecvNum == 0 || nRecvNum == SOCKET_ERROR)
		{//连接关闭
			TCHAR szErrorText[255] = { 0 };
...
// 		do 
// 		{
// 			int nToSend = (nLeng - nOffset) > 8192 ? 8192 : (nLeng - nOffset);
// 			int nSend = send(s, (const char *)&szBuffer[nOffset], nToSend, 0);
// 			if (nSend == nToSend)
// 			{
// 				nOffset += nSend;
// 				nSendCount++;
...
// 				break;
// 		} while (nOffset < nLeng);
// 		TraceMsgA("%s SendCount = %d.\n", __FUNCTION__, nSendCount);
// 		return nOffset;
// 	}
	int Send(byte *szBuffer, int nLeng)
	{
...
public:
	...
	{
		return new CSockClient(s, AddrIn);
	}
	...
	static UINT __stdcall ListenThread(void *p)
	{
		...
		while(pThis->m_bListenThreadRun)
		{
			...
			else
			{
				...
				CSockClient *pClient = pThis->CreateClient(hNewClient,ClientAddr);	
	...
	int ProcessEvent(LPThreadInfo pThreadInfo,DWORD dwIndex)
	{
		...
		{// socket is closed
			return RemoveClient(pThreadInfo,dwIndex);
		}
		return 0;
	}
	...
	void CheckFreeClient(LPThreadInfo pThreadInfo)
	{
		...
		for (int nIndex = pThreadInfo->dwEventCount - 1; nIndex >= 0;nIndex --)
		{
			...
			if (TimeSpanEx(pClient->tLastAction) >= 0.100f)
				RemoveClient(pThreadInfo, nIndex);
		}
		::LeaveCriticalSection(&pThreadInfo->csThread);
	}
	static UINT __stdcall MonitorThread(void *p)
	{
		...
		while(pThreadInfo->bRun)
		{
			...
			{//移除该客户连接
				...
				pThis->RemoveClient(pThreadInfo,dwIndex);			
			}
			else
				dwIndex ++;
	...
	int RemoveClient(LPThreadInfo pThreadInfo,DWORD dwIndex)
	{		
		WSAEVENT hClientEvent = pThreadInfo->hEventArray[dwIndex];
		CSockClient *pClient = pThreadInfo->pClientArray[dwIndex];
		::EnterCriticalSection(&pThreadInfo->csThread);
	...
	void ClearClient()
	{
		...
		for (int i = 0; i < __MaxThread; i++)
		{
			LPThreadInfo pThreadInfo = m_pThreadPool[i];
			if (pThreadInfo->hThread)
			{
				for (DWORD n = 0; n <pThreadInfo->dwEventCount; n++)
				{
					RemoveClient(pThreadInfo, n);
				}
			}
		}
